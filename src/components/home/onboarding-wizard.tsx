...
type Interest = 'programming' | 'web' | 'databases' | 'ai' | 'logic' | 'projects';
...
export default function OnboardingWizard({ locale }: Props) {
  ...
  const finish = (finalTime: Time) => {
    try {
      const prefs: GuestPrefs = { interest: interest ?? 'programming', time: finalTime };
  ...
  const interests: Interest[] = ['programming', 'web', 'databases', 'ai', 'logic', 'projects'];
  const times: Time[] = ['5', '15', '30', '60'];
  const timeKey: Record<Time, string> = { '5': 'min5', '15': 'min15', '30': 'min30', '60': 'min60' };
          ...
          <div className="flex gap-2 justify-center mb-7">
            {[0, 1].map(i => (
              <div
                ...
                style={{ width: i === step ? 28 : 8, height: 8, background: i <= step ? 'var(--zkawi-purple)' : 'var(--border)' }}
              />
            ))}
          </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                  {interests.map(it => (
                    <button
                      key={it}
                      onClick={() => { setInterest(it); setStep(1); }}
                      className="flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all"
                      style={{
                        borderColor: interest === it ? 'var(--zkawi-purple)' : 'var(--border)',
                        background: interest === it ? 'var(--zkawi-purple)/10' : 'var(--bg)',
                      }}
                    >
                      <span className="text-2xl">{INTEREST_EMOJIS[it]}</span>
                      <p className="font-bold text-xs text-center leading-tight" style={{ color: 'var(--text)' }}>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  {times.map(tv => (
                    <button
                      key={tv}
                      onClick={() => { setTime(tv); finish(tv); }}
                      className="flex flex-col items-center gap-2 p-5 rounded-2xl border-2 transition-all"
                      style={{
                        borderColor: time === tv ? 'var(--zkawi-purple)' : 'var(--border)',
                        background: time === tv ? 'var(--zkawi-purple)/10' : 'var(--bg)',
                      }}
                    >
                      <span style={{ color: 'var(--zkawi-purple)' }}>{TIME_ICONS[tv]}</span>
