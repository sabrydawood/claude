...
function markdownToHtml(markdown: string): string {
  return markdown
    ...
    .replace(
      /^- (.+)$/gm,
      '<li><span class="bullet">•</span><span>$1</span></li>',
    )
    .replace(/(<li>.*<\/li>)/gs, "<ul>$1</ul>")
    .replace(
      /^(\d+)\. (.+)$/gm,
      '<li><span class="num">$1.</span><span>$2</span></li>',
    )
    .replace(/\n\n/g, "</p><p>")
        ...
        return (
          <div key={i} className="my-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[10px] bg-[var(--zkawi-pink)]/10 text-[var(--zkawi-pink)] px-2 py-0.5 rounded-full font-bold">
                {t("activity.tryIt")}
              </span>
            </div>
            <CodePlayground
