...
const AGENT_GRADIENTS: Record<string, string> = {
  claude: "from-pink-500 to-pink-700",
...
export default function AgentPageClient({
  ...
  return (
    <main className="flex-1">
      {/* Hero */}
      <div
        className={`bg-gradient-to-br ${gradient} text-white py-12 relative overflow-hidden`}
      >
          ...
          <div className="flex items-start gap-6">
            ...
            <div>
              ...
              >
                ...
                <div className="flex items-center gap-1.5 bg-white/20 rounded-full px-3 py-1.5 text-sm font-bold">
                  <Clock size={14} />
                  {lessons.reduce((a, l) => a + l.estimatedMinutes, 0)}{" "}
                  {tL("minutes")}
                </div>
                <div className="flex items-center gap-1.5 bg-white/20 rounded-full px-3 py-1.5 text-sm font-bold">
        ...
        <div className="space-y-4">
          {lessons.map((lesson, i) => {
            ...
            return (
              <Link
                key={lesson.id}
                href={`/agents/${agentSlug}/lessons/${lesson.id}`}
              >
                {card}
              </Link>
            );
