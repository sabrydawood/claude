      ...
      >
        ...
        >
          {isPerfect ? (
            <Star
              size={72}
              className="text-[var(--zkawi-gold)]"
              fill="currentColor"
            />
          ) : isGreat ? (
            <PartyPopper size={72} className="text-emerald-500" />
          ) : isGood ? (
            <ThumbsUp size={72} className="text-[var(--zkawi-pink)]" />
          ) : (
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 bg-[var(--zkawi-pink)]/10 border border-[var(--zkawi-pink)]/30 rounded-2xl px-5 py-3 mb-6"
        >
          <Zap
            size={18}
            ...
            fill="currentColor"
          />
          <span className="font-black text-[var(--zkawi-pink)] text-lg">
            +{xpEarned} XP
          </span>
        ...
        <div className="flex gap-3 justify-center">
          ...
          <Button
            onClick={() => onComplete(finalScore, xpEarned)}
            className="gap-2"
          >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h2 className="text-xl font-black text-[var(--text)] mb-1">
          {t("title")}
        </h2>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="bg-[var(--bg-secondary)] rounded-2xl p-5 border border-[var(--zkawi-pink)]/20 mb-4">
          ...
          <div className="space-y-3">
            {currentQuestion.options.map((option) => {
              ...
              let optionStyle =
                "bg-[var(--surface)] border-2 border-[var(--border)] hover:border-[var(--zkawi-pink)] hover:bg-[var(--bg-secondary)]";

              if (showFeedback) {
                if (isCorrect) {
                  optionStyle =
                    "bg-[var(--zkawi-green)]/10 border-2 border-[var(--zkawi-green)]/50";
                } else if (isSelected && !isCorrect) {
                  optionStyle =
                    "bg-[var(--zkawi-red)]/10 border-2 border-[var(--zkawi-red)]/50";
                } else {
                  optionStyle =
                    "bg-[var(--surface)] border-2 border-[var(--border)] opacity-60";
                }
              } else if (isSelected) {
                optionStyle =
                  "bg-[var(--zkawi-pink)]/10 border-2 border-[var(--zkawi-pink)]";
              }
              ...
              >
                {answers[answers.length - 1] ? (
                  <div className="bg-[var(--zkawi-green)]/10 border border-[var(--zkawi-green)]/30 rounded-2xl p-4 flex items-center gap-3">
                    <CheckCircle2
                      size={22}
                      className="text-[var(--zkawi-green)] flex-shrink-0"
                    />
                    <div>
                      <p className="font-black text-[var(--zkawi-green)]">
                        {t("correct")}
                      </p>
                    </div>
                  </div>
