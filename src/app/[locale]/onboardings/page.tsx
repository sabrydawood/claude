  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <span
          className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${selected ? "bg-[var(--zkawi-pink)]/15 text-[var(--zkawi-pink)]" : "bg-[var(--surface-2)] text-[var(--text-muted)]"}`}
        >
          {icon}
        </span>
        <div className="flex-1 min-w-0">
          <div
            className={`font-black text-sm ${selected ? "text-[var(--zkawi-pink)]" : "text-[var(--text)]"}`}
          >
            {title}
          </div>
        ...
        {selected && (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="w-6 h-6 rounded-full bg-[var(--zkawi-pink)] flex items-center justify-center flex-shrink-0"
          >
...
export default function OnboardingPage() {
  ...
  if (isPending) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg)]">
        <Zap size={48} className="animate-bounce text-[var(--zkawi-pink)]" />
      </div>
    );
  }
  ...
  const steps = [
    // Step 0: Age
    <StepAge key="age" answers={answers} setAnswers={setAnswers} t={t} />,
    // Step 1: Goal
    <StepGoal key="goal" answers={answers} setAnswers={setAnswers} t={t} />,
    // Step 2: Experience
    <StepExperience
      key="experience"
      answers={answers}
    ...
    <StepStyle key="style" answers={answers} setAnswers={setAnswers} t={t} />,
    // Step 4: Daily time
    <StepTime key="time" answers={answers} setAnswers={setAnswers} t={t} />,
  ];
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] flex flex-col">
      ...
      <div className="h-1.5 bg-[var(--border)]">
        <motion.div
          className="h-full bg-gradient-to-r from-[var(--zkawi-pink)] to-[var(--zkawi-pink-light)] rounded-full"
              ...
              {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
                <div
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    i === step
                      ? "w-6 bg-[var(--zkawi-pink)]"
                      : i < step
                        ? "w-2 bg-[var(--zkawi-pink)]/50"
                        : "w-2 bg-[var(--border)]"
                  }`}
                />
              ))}
...
function StepHeader({
  ...
  return (
    <div className="text-center mb-8">
      <div className="flex justify-center mb-3 text-[var(--zkawi-pink)]">
        {icon}
      </div>
      ...
      <div className="space-y-3">
        {options.map((opt) => (
          <OptionCard
            ...
            onClick={() =>
              setAnswers((prev) => ({ ...prev, ageGroup: opt.value }))
            }
          />
        ))}
      </div>
...
function StepGoal({ answers, setAnswers, t }: StepProps) {
  ...
  const options: {
    ...
    },
    {
      ...
      title: st.creative,
      desc: st.creativeDesc,
    },
    {
      value: "developer",
      icon: <Code2 size={20} />,
      title: st.developer,
      desc: st.developerDesc,
    },
    {
      value: "educator",
      icon: <GraduationCap size={20} />,
      title: st.educator,
      desc: st.educatorDesc,
    },
  ];
      ...
      <div className="space-y-3">
        {options.map((opt) => (
          <OptionCard
            ...
            onClick={() =>
              setAnswers((prev) => ({ ...prev, learningStyle: opt.value }))
            }
          />
        ))}
      </div>
      ...
      <div className="space-y-3">
        {options.map((opt) => (
          <OptionCard
            ...
            onClick={() =>
              setAnswers((prev) => ({ ...prev, dailyMinutes: opt.value }))
            }
          />
        ))}
      </div>
