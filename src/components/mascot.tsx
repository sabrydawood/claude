...
const PATROLS: Record<string, Waypoint[]> = {
  ...
  dashboard: [
    { x: 74, y: 64, mood: 'happy', stayMs: 4000, message: 'patrol.dashboard.0' },
    { x: 10, y: 64, mood: 'idle', stayMs: 4000, message: 'patrol.dashboard.1' },
    { x: 40, y: 72, mood: 'thinking', stayMs: 4000, message: 'patrol.dashboard.2' },
  ],
  lessons: [
  ...
  leaderboard: [
    { x: 74, y: 55, mood: 'happy', stayMs: 4000, message: 'patrol.leaderboard.0' },
    { x: 12, y: 60, mood: 'thinking', stayMs: 4000, message: 'patrol.leaderboard.1' },
...
function RobotSVG({ mood, walking, prefersReduced }: { mood: Mood; walking: boolean; prefersReduced: boolean }) {
  return (
    <svg viewBox="0 0 80 108" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full drop-shadow-lg">
      ...
      <motion.g
        ...
        animate={prefersReduced ? {} : mood === 'happy' && !walking ? { rotate: [-30, 10, -30] } : { rotate: walking ? [-10, 10, -10] : 0 }}
        transition={prefersReduced ? {} : mood === 'happy' && !walking
      ...
      {/* Body */}
      <motion.g
        animate={prefersReduced ? {} : walking ? { rotateZ: [0, 3, 0, -3, 0] } : {}}
        transition={prefersReduced ? {} : walking ? { repeat: Infinity, duration: 0.36, ease: 'easeInOut' } : {}}
      >
      ...
      {walking ? (
        <>
          <motion.g
            ...
            transition={prefersReduced ? {} : { repeat: Infinity, duration: 0.36, ease: 'easeInOut', repeatType: 'reverse' }}
          >
...
export function Mascot() {
  ...
  useEffect(() => {
    ...
    const waypoints: Waypoint[] = isSmall
      ? [
          { x: 74, y: 75, mood: 'happy', stayMs: 5000, message: 'patrol.small.0' },
    ...
    <>
      ...
      >
        ...
        <motion.div
          ...
          animate={prefersReduced ? {} : isWalking ? { y: [0, -5, 0, -5, 0] } : { y: [0, -7, 0] }}
          transition={prefersReduced ? {} : isWalking
        ...
        >
          ...
          />
          <RobotSVG mood={mood} walking={isWalking} prefersReduced={prefersReduced} />
